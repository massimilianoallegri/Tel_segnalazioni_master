 ...
 function shell($username,$telegram,$db,$first_name,$text,$chat_id,$user_id,$location,$reply_to_msg)
{
	...
	if ($text == "/start" || $text == "info" || $text == "©️info") {
		$reply = "Benvenuto ".$first_name.". Questo Bot permette le segnalazioni di criticità sul territorio comunale. L'amministrazione non è responsabile per l'uso improprio di questo strumento e dei contenuti degli utenti. Inviando le segnalazioni si è consapevoli che l'utente e la sua segnalazione (univoci su Telegram) vengono registrati e visualizzati pubblicamente su mappa con licenza CC0 (pubblico dominio). Tali dati, realtime, si possono scaricare da https://goo.gl/vNQYPw in formato CSV.\nPuoi chiedere di partecipare all'esperimento civico compilando questo semplice form: https://goo.gl/forms/29j5UtOx3MUkxoRG2. \n\nLa geocodifca dei dati avviene grazie al database Nominatim di openStreeMap con licenza oDBL";
